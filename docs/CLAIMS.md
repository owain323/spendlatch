...
|---|---|---|---|
| C1 | Detection quality | "On 12 held-out synthetic cases with planted ground truth, the detection engine scored flag precision 1.0 and recall 1.0, with keep/hold accuracy 1.0, under a sealed two-phase protocol (predictions written before gold labels were opened)." | `benchmarks/results/metrics.json`, `benchmarks/run.py` |
...
| C9 | Benchmark integrity | "Benchmark predictions are sealed to disk before gold labels are read; the protocol is enforced by code structure, not by promise." | `benchmarks/run.py` phase comments |
